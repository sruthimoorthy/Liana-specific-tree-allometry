...
all_subplot_trees <- all_subplot_trees %>% filter(dbh >= 37)
...
# correction factor
height_cf <- exp(0.5 * resid_sd^2)

m_sum <- summary(mod_twoStage_without_subplot)
resid_sd <- m_sum$spec_pars["sigma", "Estimate"]  # or check row/col indices

# correction factor
vol_cf <- exp(0.5 * resid_sd^2)
...
post_mat_direct_allom <- exp(post_mat_direct_allom) * vol_cf
post_mat_h_allom     <- exp(post_mat_h_allom) * height_cf
...
ggsave("vol_comp_ccc_plot.png", width = 6, height = 5, plot = vol_comp_ccc_plot, dpi = 600)
